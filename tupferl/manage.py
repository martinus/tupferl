...
def add(wanted: list[str], to_host: bool, anyway: bool = False) -> int:
    ...
    for raw in wanted:
        ...
        # survivor: branch -- a real gap, and a narrow one: forcing the branch sends a *file* to
        #   `manifest.collect`, which walks it and yields nothing, so `add` on a single file would
        #   admit nothing at all. Nothing distinguishes it because every add test that passes a file
        #   also passes through `check`, which already recorded the name. Worth a test; it needs
        #   `collect`'s behaviour on a non-directory pinned first, which is a change to `manifest`
