...
def named(path: str | Path) -> Path:
    ...
    # survivor: branch -- tupferl/manifest.py:124 in named() -- the `if` is always taken --
    #   equivalent, measured: `Path.cwd() / Path('/etc/x')` is `/etc/x` -- pathlib's `/` returns the
    #   right operand when it is absolute, so taking the branch for an already-absolute path is the
    #   identity.
...
def ignored(name: PurePosixPath, patterns: Sequence[str]) -> bool:
    ...
    """
    # survivor: branch -- tupferl/manifest.py:325 in ignored() -- the `if` is never taken --
    #   equivalent: the guard is an early-out for the common case, and `any([])` is `False` -- the
    #   same answer the body computes from an empty pattern list.
    if not patterns:
