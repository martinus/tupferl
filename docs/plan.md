...
| `tupferl sync` | Pull, merge both directions, resolve, commit, push. The main command. A change *this* computer made is shown as a diff and put to the user — `[l]` store it, `[r]` discard it and take the repository's copy, `[s]` skip — before anything is written; `--auto` applies what the rules decide instead, as do `--ours`, `--theirs`, `--no-input` and a stdin that is not a terminal. An *incoming* change is applied without asking: after a settled commit-level conflict it arrives at `settle` as an ordinary inbound change, so asking would put the same file twice in one run, and `status --diff` shows what is waiting beforehand. |
