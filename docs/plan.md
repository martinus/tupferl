...
| `tupferl sync` | Pull, merge both directions, resolve, commit, push. The main command. **Every file the sync would move is shown as a diff and put to the user before anything is written**, in both directions: a change this computer made offers `[l]` store it, `[r]` discard it and take the repository's copy, `[s]` skip; an *arriving* change offers `[r]` take it and `[s]` skip, and no `[l]` — see `conflicts.ARRIVALS`. `--auto` applies what the rules decide instead, as do `--ours`, `--theirs`, `--no-input` and a stdin that is not a terminal. A file merely *missing* from `$HOME` is restored without asking, because `init` ends in a sync and reviewing restores would make bringing up a machine one keypress per file. A file settled at the commit-level prompt is not asked about again when its bytes reach `settle` — `sync.Settled` is the record that makes that true. |
