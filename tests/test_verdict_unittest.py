"""`tools/verdict_unittest.py`: which kind of failure a suite produced.

**This is the retired backend, kept until Phase C.** `tools/verdict.py` classifies
pytest reports and is what a sweep uses; this one classifies `unittest` result
objects and is reachable only with ``TUPFERL_MUTATE_VERDICT=unittest``, which
exists so a disagreement between the two can be diagnosed against the backend
that was here before. Both files and both test files are deleted together when
that is no longer wanted -- see `docs/pytest-plan.md`. The claims below are
unchanged from when this was the only backend; `tests/test_verdict.py` states
the same ones against pytest.

Issue #4 put this module first and called it a port. There is nothing to port --
`martinus/woswoar` has no test file for it; its classification is
exercised indirectly through a 117 KB `test_mutate.py`. And `tools/verdict_unittest.py`
...
#: own verdict, which is the property `verdict_unittest.py`'s docstring opens with.
SOURCE = (Path(__file__).resolve().parent.parent / "tools" / "verdict_unittest.py").read_text(
    encoding="utf-8"
)
...
def address_space_caps() -> bool:
    ...
    - it is accepted, reflected, and simply not enforced when memory is asked
      for -- which `tools/verdict_unittest.py`'s own docstring records CI discovering
...
class TestABrokenModuleTakesTwoDifferentPaths(Probe):
    ...
    #: draft did the latter, and would have passed against a `verdict_unittest.py` where
...
class TestWhenTheToolItselfCannotRun(Probe):
    ...
    loaded" from an absent report -- which is also what a typo in `verdict_unittest.py`
    ...
    **This class only runs where `RLIMIT_AS` is usable, which today means
    ...
    def limits(self, limit: int, soft: int | None = None) -> int:
        ...
        *during a sweep* the process running these tests already holds a finite
        ...
        fails on an unmutated tree: every row of a `tools/verdict_unittest.py` sweep then
        ...
        **It raises to a bounded number, never to `hard`.** `(hard, hard)` was
        ...
        code = (
            ...
            "from tools import verdict_unittest as verdict\n"
            f"verdict.cap({limit})\n"
...
class TestTheWalkPastTheSelection(Probe):
    ...
    def test_it_reaches_a_module_the_selection_never_named(self) -> None:
        ...
        # turned the `test (3.10)` leg red -- and `verdict_unittest.py` says why in the
...
class TestWhereTheWalkLooks(unittest.TestCase):
    ...
    **Imported here, unlike everything else in this file, and the module
    ...
    def test_it_follows_the_package_the_selection_lives_in(self) -> None:
        ...
        self.assertIn("tests.test_verdict_unittest", found)
        self.assertNotIn("test_verdict_unittest", found, "the package prefix was dropped")
    ...
    def test_a_flat_selection_looks_beside_itself(self) -> None:
        ...
        # `tools/verdict_unittest.py` against its own tests at all, and reported
        # `BASELINE NOT GREEN` for every table that tried.
        from tools import verdict_unittest  # noqa: F401
