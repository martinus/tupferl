...
added (see `run_watch`); and `tools/watch.py` gained `SHOUT` and `tint`, which
colour its four answers for a terminal -- `TestEveryAnswerIsColoured` below.

**And the import claim went with them, in B5.** This module now imports
`tests.support`, for two things it was reimplementing: `bounded`, without which
`BOUND` beats only the *default* alarm and not the one a sweep armed, and
`Screen`, a `StringIO` that says it is a terminal. Both were the reason to reach
across rather than a convenience -- see `BOUND` and `TestEveryAnswerIsColoured`.
...
#: `tools/mutate.py`'s per-test alarm -- see `run_watch` for why both bounds
#: matter and why equalling the second one is useless.
#:
...
#:
#: **Through `support.bounded`, which is what B5 came here to do.** A bare 20
#: beats the *default* 30s alarm and nothing else: a sweep at `--each-test 10`
#: puts this bound back above the alarm, the two race, the alarm wins, and the
#: row is filed `BROKE` -- which is never `caught`, so the seven `main` and
#: `alive` rows this constant exists to guard would be guarded by nothing again.
#: That is the identical hole `support.bounded`'s docstring records for
#: `PROMPTED`, and it is the reason this module now imports `tests.support` at
#: all.
...
def box() -> Iterator[Box]:
    with support.tempdir(prefix="tupferl-watch-") as root:
        yield Box(root, root / "run.log", root / "report.json")


@pytest.fixture
def running() -> Iterator[subprocess.Popen[bytes]]:
...
def watcher(box: Box, pid: int, match: str = ".", stale: float = watch.STALE) -> watch.Watch:
    """A `Watch` over `box`'s two files.

    Named for the thing rather than for the verb, because the tests below bind
    what it returns to a local called `watching` -- and a module-level function
    of that name would make every one of them read an unbound local. That is the
    B4b lesson about a method becoming a function, met head on rather than found
    by ruff.
    """
    return watch.Watch(pid, box.log, box.done, re.compile(match), stale)


def stuck_for(box: Box, seconds: float, stale: float = 60.0) -> watch.Watch:
    ...
    """
    watching = watcher(box, os.getpid(), stale=stale)
    watching.step()
...
def command(box: Box, *extra: str) -> list[str]:
    return [
        sys.executable,
        "-m",
        "tools.watch",
...
def run_watch(box: Box, *extra: str) -> subprocess.CompletedProcess[str]:
    ...
    **The number has to beat the harness's, not merely exist.** It was 30
    ...
    that branch: 7 BROKE at 30s.

    `BOUND` sits above the longest honest wait here and below the alarm --
    below whatever alarm is *armed*, since B5 put it through
    `support.bounded`. The two tests that wait for a pidfile that never comes
    take just over 10s by design, so anything at or under that would fail them
...
def parsed() -> ast.Module:
    """`tools/watch.py` as code, not as text.
...
def coloured(watching: watch.Watch) -> str:
    """One real event, painted for a terminal."""
    line, _ = watching.step() or ("", 0)
    assert line, "the fixture produced no event to colour"
    return watch.tint(line, support.Screen())


def leading_codes(painted: str) -> str:
    """The run of codes `painted` starts with, and nothing after it."""
    found = OPENING.match(painted)
    assert found is not None, painted.encode()
    return found.group()


class TestWhetherAProcessIsThere:
    def test_this_process_is_alive(self) -> None:
        assert watch.alive(os.getpid())

    def test_a_reaped_process_is_not(self) -> None:
        assert not watch.alive(reaped())

    def test_asking_does_not_disturb_the_job(self, running: subprocess.Popen[bytes]) -> None:
    ...
    def test_pid_one_is_alive_though_it_is_not_ours(self, running: subprocess.Popen[bytes]) -> None:
        ...
        try:
            ...
            pytest.skip("no pid 1 to ask about")
        else:
            pytest.skip("this process may signal pid 1, so the branch is not reached")
        assert watch.alive(1)


class TestAPidThatIsNotOne:
    ...
    def test_the_message_says_what_is_wrong_with_it(self) -> None:
        ...
        with pytest.raises(ValueError) as raised:
            watch.alive(0)
        assert "process group" in str(raised.value)

    def test_an_ordinary_pid_still_gets_through(self) -> None:
        """The guard above passes on a converter that rejects everything."""
        assert os.getpid() == watch.a_pid(str(os.getpid()))


class TestTheJobIsGone:
    """The failure the tool was written for: a death that looked like patience."""

    def test_a_dead_pid_is_reported_dead(self, box: Box) -> None:
        line, status = watcher(box, reaped()).step() or ("", -1)
        assert "DIED" in line
        assert status == 1

    def test_it_says_gone_rather_than_slow(self, box: Box) -> None:
        """The wording is the point. "No report yet" reads as something to wait
        out, which is exactly what the reader did for an hour."""
        line, _ = watcher(box, reaped()).step() or ("", -1)
        assert "gone, not slow" in line

    def test_it_says_how_long_the_job_had_been_running(self, box: Box) -> None:
        ...
        """
        watching = watcher(box, reaped())
        watching.began -= 3600.0
        line, _ = watching.step() or ("", 0)
        assert "after 60m" in line

    def test_it_says_how_far_the_job_got(self, box: Box) -> None:
        box.log.write_text("row\nrow\nrow\n", encoding="utf-8")
        line, _ = watcher(box, reaped()).step() or ("", -1)
        assert "3 rows" in line


class TestAFinishedJobIsNotADeadOne:
    ...
    def test_a_report_from_a_process_that_has_exited_is_a_finish(self, box: Box) -> None:
        box.done.write_text("{}", encoding="utf-8")
        line, status = watcher(box, reaped()).step() or ("", -1)
        assert "FINISHED" in line
        assert status == 0

    def test_without_the_report_the_same_pid_is_a_death(self, box: Box) -> None:
...
class TestWhatCountsAsProgress:
    def test_the_first_look_is_an_event_even_at_zero(self, box: Box) -> None:
        ...
        line, status = watcher(box, os.getpid()).step() or ("", 0)
        assert (line, status) == ("working: 0 rows after 0m", -1)

    def test_a_log_that_already_has_rows_still_gets_an_opening_line(self, box: Box) -> None:
        ...
        box.log.write_text("row\n", encoding="utf-8")
        line, status = watcher(box, os.getpid()).step() or ("", 0)
        assert (line, status) == ("working: 1 rows after 0m", -1)

    def test_an_unchanged_count_is_not_an_event(self, box: Box) -> None:
        """One line per twenty seconds for forty minutes is the same as no
        signal, by a different route."""
        watching = watcher(box, os.getpid())
        watching.step()
        assert watching.step() is None

    def test_a_changed_count_is(self, box: Box) -> None:
        watching = watcher(box, os.getpid())
        watching.step()
        box.log.write_text("row\n", encoding="utf-8")
        line, _ = watching.step() or ("", 0)
    ...
    def test_only_matching_lines_are_counted(self, box: Box) -> None:
        box.log.write_text("caught one\nnoise\ncaught two\n", encoding="utf-8")
        line, _ = watcher(box, os.getpid(), match="^caught").step() or ("", 0)
        assert "2 rows" in line

    def test_a_log_holding_bytes_that_are_not_utf8_is_still_counted(self, box: Box) -> None:
...
class TestAJobCanStopWithoutEnding:
    ...
    def test_a_long_enough_silence_is_reported(self, box: Box) -> None:
        line, status = stuck_for(box, 120.0).step() or ("", 0)
        assert "STALLED" in line
        assert status == -1

    def test_it_says_the_process_is_still_there(self, box: Box) -> None:
        ...
        line, _ = stuck_for(box, 120.0).step() or ("", 0)
        assert "alive and not working" in line

    def test_it_says_how_long_and_how_far(self, box: Box) -> None:
        line, _ = stuck_for(box, 120.0).step() or ("", 0)
        assert "120s" in line
        assert "0 rows" in line

    def test_a_short_silence_is_not(self, box: Box) -> None:
        ...
        with it."""
        assert stuck_for(box, 30.0).step() is None

    def test_it_does_not_repeat_itself_every_poll(self, box: Box) -> None:
        ...
        watching = stuck_for(box, 120.0)
        assert watching.step() is not None
        assert watching.step() is None

    def test_it_speaks_again_when_the_stall_doubles(self, box: Box) -> None:
    ...
    def test_progress_starts_the_clock_again(self, box: Box) -> None:
    ...
    def test_a_recovered_job_stalls_again_on_its_own_terms(self, box: Box) -> None:
    ...
    def test_zero_turns_it_off(self, box: Box) -> None:
        ...
        assert stuck_for(box, 9999.0, stale=0.0).step() is None

    def test_a_finish_outranks_a_stall(self, box: Box) -> None:
...
class TestEveryAnswerIsColoured:
    ...
    That stream is `support.Screen`, which is where this module's import of
    `tests.support` began. It used to be a nested class of the same name, with a
    docstring that was `support.Screen`'s own argument written out a second time
    -- two copies of "a `StringIO` that claims `isatty`", free to drift.
    """

    def test_the_four_answers_get_four_different_colours(self, box: Box) -> None:
        ...
        box.done.write_text("{}", encoding="utf-8")
        finished = coloured(watcher(box, reaped()))
        box.done.unlink()
        died = coloured(watcher(box, reaped()))
        stalled = coloured(stuck_for(box, 120.0))
        working = coloured(watcher(box, os.getpid()))

        for painted in (finished, died, stalled, working):
            # A plain loop rather than `parametrize`: the four lines are built
            # by this test, so a parametrized version would build all four in
            # each of four cases. Four literals, so the zero-iteration trap
            # CLAUDE.md names does not apply.
            assert painted.startswith("\x1b["), f"an answer went unpainted: {painted.encode()!r}"
            assert painted.endswith(paint.OFF), painted.encode()
        ...
        # for this change.
        opening = [leading_codes(said) for said in (finished, died, stalled, working)]
        assert len(set(opening)) == 4, opening

    def test_the_words_are_still_greppable(self, box: Box) -> None:
    ...
    def test_a_reworded_answer_simply_goes_unpainted(
        ...
        assert watch.tint("running: 3 rows", support.Screen()) == "running: 3 rows"


class TestItForksNothing:
    ...
    @pytest.mark.parametrize("forbidden", ("subprocess", "multiprocessing", "pty"))
    def test_it_imports_nothing_that_starts_a_process(self, forbidden: str) -> None:
        imported = {
            name.name.split(".")[0]
    ...
    @pytest.mark.parametrize("forbidden", ("system", "popen", "spawnv", "fork"))
    def test_it_calls_nothing_that_starts_a_process(self, forbidden: str) -> None:
        ...
        bring a shell back."""
        reached = {node.attr for node in ast.walk(parsed()) if isinstance(node, ast.Attribute)}
    ...
    def test_the_guard_can_actually_fail(self) -> None:
        ...
        reached = {node.attr for node in ast.walk(parsed()) if isinstance(node, ast.Attribute)}
        assert "kill" in reached


class TestTheCommandLine:
    def test_it_exits_zero_when_the_job_finished(self, box: Box) -> None:
        box.done.write_text("{}", encoding="utf-8")
        ran = run_watch(box, str(reaped()))
        assert ran.returncode == 0, ran.stderr
        assert "FINISHED" in ran.stdout

    def test_it_exits_one_when_the_job_died(self, box: Box) -> None:
        """The exit status matters as much as the line: a caller that only reads
        the status still learns the difference."""
        ran = run_watch(box, str(reaped()))
        assert ran.returncode == 1
        assert "DIED" in ran.stdout

    def test_it_repeats_nothing_while_nothing_changes(
        self, box: Box, running: subprocess.Popen[bytes], request: pytest.FixtureRequest
    ) -> None:
        ...
        assert burned < 0.5, f"a second of watching cost {burned:.2f}s of CPU"

    def test_the_stall_threshold_reaches_the_watch(
        self, box: Box, running: subprocess.Popen[bytes], request: pytest.FixtureRequest
    ) -> None:
    ...
    def test_it_reads_the_pid_the_job_wrote(
        ...
        """
        box.done.write_text("{}", encoding="utf-8")
        pidfile = box.root / "sweep.pid"
        pidfile.write_text(f"{reaped()}\n", encoding="utf-8")
        ran = run_watch(box, "--pidfile", str(pidfile))
        assert ran.returncode == 0, ran.stderr
        assert "FINISHED" in ran.stdout

    def test_it_waits_for_a_pidfile_that_is_not_there_yet(
        self, box: Box, request: pytest.FixtureRequest
    ) -> None:
        ...
        request.addfinalizer(writer.wait)
        request.addfinalizer(writer.kill)
        ran = run_watch(box, "--pidfile", str(pidfile))
        assert ran.returncode == 0, ran.stderr

    def test_a_long_poll_interval_does_not_stall_the_wait(
        self, box: Box, request: pytest.FixtureRequest
    ) -> None:
        ...
        ran = run_watch(box, "--pidfile", str(pidfile), "--interval", "20")
        assert ran.returncode == 0, ran.stderr
        assert time.monotonic() - began < 5.0, "it waited a whole poll interval"

    def test_waiting_for_a_pidfile_is_not_a_spin(self, box: Box) -> None:
        ...
        assert ran.returncode == 1
        burned = resource.getrusage(resource.RUSAGE_CHILDREN).ru_utime - before
        assert burned < 2.0, f"waiting cost {burned:.2f}s of CPU"

    def test_a_pidfile_that_never_arrives_is_an_error_not_a_wait(self, box: Box) -> None:
    ...
    def test_the_deadline_defaults_to_the_constant(self, box: Box) -> None:
        ...
        """
        ran = run_watch(box, "--pidfile", str(box.root / "never"), "--pidfile-wait", "0.05")
        assert ran.returncode == 1
        assert "after 0.05s" in ran.stderr
        assert watch.PIDFILE_WAIT == 10.0

    def test_the_constant_is_resolved_when_it_is_read(self, box: Box) -> None:
        ...
        with (
            ...
            watch._await_pid(box.root / "never", 0.05)
        assert "after 0.05s" in str(caught.value)

    @pytest.mark.parametrize("said", ("0.05", "0.25"))
    def test_the_deadline_it_reports_is_the_one_it_was_given(self, said: str, box: Box) -> None:
        ...
        ran = run_watch(box, "--pidfile", str(box.root / "never"), "--pidfile-wait", said)
        assert f"after {float(said):g}s" in ran.stderr

    def test_a_pid_and_a_pidfile_together_are_refused(self, box: Box) -> None:
        """Two answers to one question, and no way to tell which the caller
        meant. argparse says so before anything runs."""
        ran = run_watch(box, "123", "--pidfile", str(box.root / "p"))
        assert ran.returncode == 2

    def test_neither_a_pid_nor_a_pidfile_is_refused(self, box: Box) -> None:
        ran = run_watch(
            box,
        )
        assert ran.returncode == 2

    def test_a_pid_that_is_a_group_is_a_usage_error(self, box: Box) -> None:
        ran = run_watch(box, "0")
        assert ran.returncode == 2
        assert "process group" in ran.stderr
