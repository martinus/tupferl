...
#: A guarded class and a plain function beside it, so every claim below can be
#: made in both directions at once. "Nothing was generated" is what a broken
#: exclusion and a working one both look like from one half.
GUARDED = """\
class Guard:
    def keep(self, a, b):
        return a < b
...
#: Every line of `GUARDED`, since these tests are about scope and not about a diff.
ALL_OF_IT = set(range(1, 10))


class TestTheGuardAProbeNeedsIsNotMutated(unittest.TestCase):
    """#96: a probe cannot answer a row that disables the bound it runs under.

    A sweep copies the tree, mutates it and runs this suite inside the copy --
    and this suite drives nested harnesses. Mutate the memory guard or the
    sandbox pool and the probe below has neither, so the row comes back `BROKE`,
    which is never `caught`. Measured across two 1030-row sweeps differing only
    in ceiling: 8 of 9 unanswered rows recurred, all of them in that machinery.

    `mutants.UNBOUNDED` refuses to generate them. These tests hold both halves
    of what that has to be worth having: it must take away the rows it names,
    and it must take away nothing else -- and it must say so, because a table
    that quietly shrinks looks exactly like one that was always this size.
    """

    def test_a_named_scope_generates_nothing_and_its_neighbour_still_does(self) -> None:
        with mock.patch.dict(mutants.UNBOUNDED, {"w/t.py": ("Guard",)}):
            rows = mutants.generate(GUARDED, "w/t.py", ALL_OF_IT, tests="t")
        self.assertEqual(
            [row.label for row in rows],
            [
                "w/t.py:7 in free() -- `<` becomes `<=`",
                "w/t.py:7 in free() -- returns `None` instead of `a < b`",
            ],
        )

    def test_a_class_is_excluded_whole(self) -> None:
        """The scope, not the line the name is on.

        `_Lanes` is named as a class because every method of it is machinery the
        probe depends on, and a list of lines would need re-checking against the
        file on every edit to it. So the method body has to go with the header,
        and asserting on the header alone would pass for a rule that only
        blocked line 1.
        """
        with mock.patch.dict(mutants.UNBOUNDED, {"w/t.py": ("Guard",)}):
            blocked = mutants.unbounded(GUARDED, "w/t.py")
        self.assertEqual(blocked, {1, 2, 3})

    def test_one_method_can_be_named_without_taking_its_class(self) -> None:
        """`Work.take` is named that way, and `_attempt` is not named at all.

        Excluding a whole function to reach one statement in it is what the
        `_lent` helper in `tools/mutate.py` exists to avoid -- `_attempt` decides
        the measured `first`/`Learned` row ordering, which is exactly what wants
        mutating.
        """
        with mock.patch.dict(mutants.UNBOUNDED, {"w/t.py": ("Guard.keep",)}):
            self.assertEqual(mutants.unbounded(GUARDED, "w/t.py"), {2, 3})

    def test_what_was_refused_is_reported(self) -> None:
        with mock.patch.dict(mutants.UNBOUNDED, {"w/t.py": ("Guard",)}):
            gone = mutants.refused(GUARDED, "w/t.py", ALL_OF_IT, tests="t")
        self.assertEqual(
            [row.label for row in gone],
            [
                "w/t.py:3 in Guard.keep() -- `<` becomes `<=`",
                "w/t.py:3 in Guard.keep() -- returns `None` instead of `a < b`",
            ],
        )

    def test_a_file_nobody_named_is_untouched(self) -> None:
        rows = mutants.generate(GUARDED, "w/other.py", ALL_OF_IT, tests="t")
        self.assertEqual(
            [row.label for row in rows],
            [
                "w/other.py:3 in Guard.keep() -- `<` becomes `<=`",
                "w/other.py:3 in Guard.keep() -- returns `None` instead of `a < b`",
                "w/other.py:7 in free() -- `<` becomes `<=`",
                "w/other.py:7 in free() -- returns `None` instead of `a < b`",
            ],
        )
        self.assertEqual(mutants.refused(GUARDED, "w/other.py", ALL_OF_IT), [])

    def test_a_pragma_is_not_counted_as_a_refusal(self) -> None:
        """A line the author had already silenced is not a row the guard took away.

        Both passes honour the pragma, so the difference between them is the
        exclusion alone. Counting it as well would inflate the note `generated`
        prints -- in the direction that reads as the guard earning its place.
        """
        source = GUARDED.replace(
            "        return a < b", "        return a < b  # pragma: no mutate", 1
        )
        with mock.patch.dict(mutants.UNBOUNDED, {"w/t.py": ("Guard",)}):
            self.assertEqual(mutants.refused(source, "w/t.py", ALL_OF_IT, tests="t"), [])

    def test_a_name_that_has_gone_is_refused_loudly(self) -> None:
        """The rot this is exposed to, and the only direction it rots in.

        A name-keyed exclusion stops matching the moment somebody renames the
        thing, and then the rows come back -- every one of them `BROKE`, which
        is never `caught`, so nothing in the output objects. `SystemExit` is
        `_chosen`'s answer to the same shape one layer up.
        """
        with (
            mock.patch.dict(mutants.UNBOUNDED, {"w/t.py": ("Guard", "Vanished")}),
            self.assertRaises(SystemExit) as loudly,
        ):
            mutants.generate(GUARDED, "w/t.py", ALL_OF_IT)
        self.assertIn("Vanished", str(loudly.exception))
        self.assertNotIn("Guard,", str(loudly.exception))


class TestTheExclusionAgainstTheRealTree(unittest.TestCase):
    """The half no synthetic fixture can check: the names still name something.

    Here rather than in a sweep, because a sweep of the file this governs is
    hours and this is milliseconds -- and because the failure it looks for is
    silent everywhere else.
    """

    def test_every_name_covers_lines_of_the_file_it_names(self) -> None:
        for path, names in sorted(mutants.UNBOUNDED.items()):
            source = (REPO_ROOT / path).read_text(encoding="utf-8")
            for name in names:
                with (
                    self.subTest(path=path, name=name),
                    mock.patch.dict(mutants.UNBOUNDED, {path: (name,)}),
                ):
                    # Asked one name at a time: together they would pass with a
                    # name resolving to nothing, as long as some other one did.
                    self.assertTrue(mutants.unbounded(source, path))

    def test_no_disposition_is_left_where_no_row_can_reach_it(self) -> None:
        """A `# survivor:` tag inside an excluded scope excuses nothing.

        It cannot even be reported spent: `Survivors` judges a tag against the
        rows that ran, and here none is generated. So it would sit there
        claiming a survivor had been read, for ever, silently -- which is the
        mute list the tag design exists to avoid, arriving by a route the
        design did not anticipate. Fourteen were deleted with this exclusion;
        this is what stops the fifteenth.
        """
        for path in sorted(mutants.UNBOUNDED):
            source = (REPO_ROOT / path).read_text(encoding="utf-8")
            guarded = mutants.unbounded(source, path)
            left = [
                f"{path}:{number}"
                for number, line in enumerate(source.split("\n"), 1)
                if number in guarded and mutants.TAGGED.search(line)
            ]
            with self.subTest(path=path):
                self.assertEqual(left, [])

    def test_the_rows_that_remain_are_all_outside_and_the_rows_that_went_all_inside(
        self,
    ) -> None:
        """Both directions, against the lines rather than against the difference.

        `refused` is defined as what one generation has and another does not, so
        checking it that way would restate its implementation. `Offsets.line_of`
        is an independent route to the same claim.
        """
        for path in sorted(mutants.UNBOUNDED):
            with self.subTest(path=path):
                source = (REPO_ROOT / path).read_text(encoding="utf-8")
                ...
                guarded = mutants.unbounded(source, path)
                offsets = mutants.Offsets(source)

                def spanned(row: Mutation, offsets: mutants.Offsets = offsets) -> set[int]:
                    start, end = row.span or (0, 0)
                    return set(range(offsets.line_of(start), offsets.line_of(end) + 1))

                kept = mutants.generate(source, path, whole)
                gone = mutants.refused(source, path, whole)
                self.assertTrue(gone)
                self.assertTrue(kept)
                self.assertEqual([row.label for row in kept if spanned(row) & guarded], [])
                self.assertEqual([row.label for row in gone if not spanned(row) & guarded], [])
