...
| `tupferl/colours.py` | every escape sequence the package writes, and the one question that decides whether to use one: is *this stream* a terminal. Not named `paint`, which is `tools/`'s and would collide with its test module; not merged with it either, since `tools/` must not import the package it is meant to be able to break |
